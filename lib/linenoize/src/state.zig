...
fn compareLeft(_: []const u8, avail_space: usize, index: usize) math.Order {
    return math.order(avail_space, index);
}

fn compareRight(_: []const u8, avail_space: usize, index: usize) math.Order {
    return math.order(index, avail_space);
}
...
pub const LinenoiseState = struct {
    ...
    fn refreshSingleLine(self: *Self) !void {
        ...
        // Calculate widths
        const pos = self.pos;
        const prompt_width = self.prompt.len;
        const hint_width = if (hint) |str| str.len else 0;
        const buf_width = self.buf.items.len;
